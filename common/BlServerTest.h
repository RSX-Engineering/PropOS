  ...
  void Loop() //override 
  {
    if (deviceConnected) {
        ...
        if(parseCmd) {     // add semaphore here !!! 
            parseCmd = false;
            if(!strcmp(param, "board_info"))
            {
              strcpy((char*)&txValue[0], "board_info-START");
              strcpy((char*)&txValue[18], "SaberProp");
              strcpy((char*)&txValue[18+13], "board_info-END");
              pTxCharacteristic->setValue((uint8_t*)&txValue[0], 18+13+15);
              pTxCharacteristic->notify();  
              return;
            }
