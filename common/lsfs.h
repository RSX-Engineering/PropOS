...
#else
#define DOSFS_SDMMC_CMD   42
#define DOSFS_SDMMC_D0    47
#define DOSFS_SDMMC_D1    3
#endif
#define DOSFS_SDMMC_D2    39
#define DOSFS_SDMMC_D3    40
#else 
#define DOSFS SD_MMC
#define DOSFS_SDMMC_CLK   38
#define DOSFS_SDMMC_CMD   37
...
public:
  ...
  static bool Begin() {
    ...
    if (!DOSFS.begin("/sdcard", false, false, SDMMC_FREQ_HIGHSPEED, 8)) return false; //20000    SDMMC_FREQ_HIGHSPEED SDMMC_FREQ_DEFAULT
    #elif defined(PROFFIE_ESP_SDSPI)
  ...
  static void End() {
    if (!mounted_) return;
    DOSFS.end();
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_D3);
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_D2);
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_D1);
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_D0);
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_CMD);
    gpio_reset_pin((gpio_num_t)DOSFS_SDMMC_CLK);
  ...
  static File Open(const char* path)  __attribute__((optimize("Ofast"))) {
    ...
    openTime = micros();
    correctPath(_correctedPath, path);
    File f = DOSFS.open(_correctedPath, "rb");
  ...
  static File OpenForOverWrite(const char* path) {
    if (!mounted_) return File();
    correctPath(_correctedPath, path);
    File f = DOSFS.open(_correctedPath, "rb+");     // was r+
    return f;
  }
  ...
  static File OpenForWrite(const char* path) {
    if (!mounted_) return File();
    correctPath(_correctedPath, path);
    File f = DOSFS.open(_correctedPath, "wb");
    if (!f) {
      PathHelper tmp(_correctedPath);
      if (tmp.Dirname()) {
	DOSFS.mkdir(tmp);
	f = DOSFS.open(_correctedPath, "wb");
      }
    }
...
public:
  ...
  static bool Exists(const char* path) {
    if (!mounted_) return false;
    return DOSFS.exists(path);   // was path 
  }
  static bool Remove(const char* path) {
    if (!mounted_) return false;
    return DOSFS.remove(path);
  ...
  static File Open(const char* path)  __attribute__((optimize("Ofast"))) {
    if (!mounted_) return File();
    openTime = micros();
    File f = DOSFS.open(path, FILE_READ);
    openTime = micros() - openTime;
    return f;
  }
  ...
  static File OpenRW(const char* path) {
    if (!mounted_) return File();
    File f = DOSFS.open(path, FILE_WRITE);
    if (!f) f = OpenForWrite(path);
    return f;
  }
  ...
  static File OpenForOverWrite(const char* path) {
    if (!mounted_) return File();
    File f = DOSFS.open(path, FILE_WRITE);     // was r+
    return f;
  }
