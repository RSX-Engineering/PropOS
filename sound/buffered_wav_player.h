...
bool PlayEffect(Effect* soundEffect, int soundID=-1, uint16_t repeat=0, float volume=0) {
  ...
  void ChangeRepeatTime(uint16_t milli) {
    ...
    if (wav.longRepeat && *(wav.longRepeat)) { // currently on long repeat 
        if (milli == 1 || milli == soundDuration) {
            ...
            if(wav.longRepeat)*(wav.longRepeat) = false ;    // stop RepeatTask
            // repeater->Stop();
        }
        ...
        if (milli == 1 || milli == soundDuration) {
            ...
            wav.longRepeat = &repeater->running;      // magic glue
            if(wav.longRepeat)*(wav.longRepeat) = true;                 // also starts repeater
            // repeater->Start();
        }
        else {
  ...
  void FadeAndStop(uint16_t speed = 125) {
    ...
    wav.shortRepeatTime = 0;
    if(repeater){
      if(wav.longRepeat)*(wav.longRepeat) = false;
    }
