...
// First some abstractions for controlling PWM pin
  ...
  { 
    ...
    ledc_channel.flags.output_invert = 0b0;
    ...
    // STDOUT.print("write setup ");STDOUT.println(pin);
    // nrActiveChannel++;
  }
  ...
  void LSanalogWriteTeardown(uint32_t pin) 
  {
    ...
    activeChannels.state[channelOFPin] = 0;      // freee channel because it was not init OKAY , to default values
    activeChannels.channel[channelOFPin] = 0xFF; // to default values
    activeChannels.pin[channelOFPin] = 0xFF;     // cto default values
    pinMode((uint8_t)pin, PULLDOWN);
    // STDOUT.print("tearingdown ");STDOUT.println(pin);

    if(activeChannels.state[0] == 0 && activeChannels.state[1] == 0 &&  activeChannels.state[2] == 0 && activeChannels.state[3] == 0)
    {
      ledc_timer_rst(LEDC_MODE, LEDC_TIMER);
      isInit = false;
      // STDOUT.println("reseting timer ");
    }
  ...
  void LSanalogWriteSetup(uint32_t pin) 
  {
    ...
    if(!MyTim) {
      ...
      if(!MyTim) return; // failed init timer , something went terrible wrong 
      uint32_t targetTimerFreq = 26666666; // in Hz
      uint32_t autoreload = 32768;         // we want that control register to be 0 - 32768 
      // PWM freq targeted is 26666666/32768 = 813 Hz
      uint32_t prescalerFactor = MyTim->getTimerClkFreq() / targetTimerFreq;
      LL_TIM_SetPrescaler(Instance, prescalerFactor - 1);
      MyTim->setOverflow(autoreload, TICK_FORMAT);
    }

    // Configure and start PWM
    // MyTim->setPWM(channel, pin, 5, 10, NULL, NULL); // No callback required, we can simplify the function cal
    if(MyTim) {
      MyTim->setMode(channel, TIMER_OUTPUT_COMPARE_PWM1, pin);
      MyTim->setCaptureCompare(channel, 0, TICK_COMPARE_FORMAT);
      MyTim->resumeChannel(channel);  // start pwm gen on 0 
      activeChannel |= (1 << channel);
    }
  ...
  void LSanalogWriteTeardown(uint32_t pin) 
  {
    ...
    uint32_t channel = STM_PIN_CHANNEL(pinmap_function(digitalPinToPinName(pin), PinMap_PWM));
    if(MyTim) {
      MyTim->pauseChannel(channel);  // disable channel
      activeChannel &= ~(1 << channel);
    }

    if(!activeChannel) {
      MyTim->timerHandleDeinit();
      delete MyTim; // can deinit timer 
    }
  }
