...
private:
    ...
    void freeBusySPI(spi_host_device_t dev)
    {
        if(!dev) return;
        spiPeriphState[dev-1] = 0;
    }
public:
    void deinit()
    {
        while(!IsReadyForEndFrame());    // wait transaction to end TODO add timeout ms 
        freeBusySPI(SPI_settings.host);
    ...
    bool init() {
        ...
        SPI_settings.buscfg.mosi_io_num = pin_;
