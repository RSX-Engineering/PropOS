...
enum emojiSounds_id {     
    ...
    okdone = 22,
    emojiMAX = okdone         // update if needed!
#else 
...
enum menuVoices_id {    
    ...
    longColor = 3,          // 'color' spoken, pre-mixed with menu annunciator
    brightness = 4,         // 'brightness' spoken
    volume = 5,             // 'volume' spoken
    longVolume = 6,         // 'volume' spoken, pre-mixed with menu annunciator
    sensitivity = 7,        // 'sensitivity' spoken
    soundfont = 8,
    longSoundfont = 9,
    soundtrack = 10,
    preset = 11,
    combatsaber = 12,
    katasaber = 13,
    notrack = 14,           // 'no track' option for track menu


    menuMAX = notrack   // update if needed!
};
...
// NAV HELPERS:
    ...
    void StartScroll(uint32_t strength) {
        ...
        #ifdef DIAGNOSE_MENU    
            STDOUT.print("START scrolling , strength = ");  STDOUT.print(strength);
            STDOUT.print(", theta = "); STDOUT.print(fusor.theta());
            STDOUT.print(", thetaPeak = "); STDOUT.print(fusor.thetaPeak());   
            ...
            if (scrollIndex) {   // scroll active
                ...
                if ((fusor.thetaPeak() > MENU_STARTSCROLL) && (signof<int32_t>(fusor.theta()) == signof<int16_t>(scrollIndex)) && (timeNow-lastScrollTime >= MENU_SCROLLPAUSE)) {
                    ...
                    if (fusor.thetaPeak() - MENU_STARTSCROLL > abs(scrollIndex)) {
                        #ifdef DIAGNOSE_MENU  
                            STDOUT.print("RESTART scrolling... ");
                        #endif  
