    ...
    uint32_t FileCrC(uint32_t* crcRead) //__attribute__((optimize("Og")))
    {
       ...
       LOCK_SD(true);
        ...
        for(uint32_t indx = 0 ; indx < toRead; indx += 4)
        {
            ...
            #else
            if(indx == 0)
                crcCalculated = STCrc_impl.CRC_Calculate(&localStorVar[0], 4); // reset crc and calculate      4 
            else
                crcCalculated = STCrc_impl.CRC_Acumulate(&localStorVar[0], 4); // accumulate       4 
            #endif
        }
        if(crcRead) {
            pFile->read((uint8_t*)&localStorVar[0], 4);
...
class CodInterpreter_V3 : public CodInterpreter_V2 {
    ...
    */
    uint32_t BinCrc(uint32_t fileOffset, uint32_t size)
    {
        ...
        LOCK_SD(true);
        #if defined(ARDUINO_ARCH_STM32L4) || defined(ARDUINO_ARCH_STM32U5)   // STM architecture                                      // lock SD - so operation wont be disturbed 
        curPos = pFile->position();
        if(curPos != fileOffset)                 // position in file at desired offset 
            if(!pFile->seek(fileOffset)) return 0;
        #else 
        curPos = pFile->position();
        if(!pFile->seek(fileOffset, SeekSet)) return 0;
        if(pFile->position() != fileOffset)                 // position in file at desired offset 
            return 0;
        #endif          
        ...
        for(uint32_t indx = 0 ; indx < size; indx += 4)
        {
            ...
            #else
            if(indx == 0)
                crcCalculated = STCrc_impl.CRC_Calculate((uint32_t*)&localStorVar[0], 4); // reset crc and calculate      4 
            else
                crcCalculated = STCrc_impl.CRC_Acumulate((uint32_t*)&localStorVar[0], 4); // accumulate       4 
            #endif
        }
        #if defined(ARDUINO_ARCH_STM32U5)
        pFile->seek(curPos);
        #else
        pFile->seek(curPos, SeekSet);       // reset file position at find position 
        #endif 
        // result = this->ReadHeader(); TODO check out this
        ...
        if(result > 0) 
        {
            ...
            if(file.position() != codInterpreter->currentCodOffset) // check if position is located at desired offset 
                if(!file.seek(codInterpreter->currentCodOffset))    // set desired offset 
                    return -4;
            #else
            if(!file.seek(codInterpreter->currentCodOffset, SeekSet))    // set desired offset 
                    return -4;
            if(file.position() != codInterpreter->currentCodOffset)
                    return -4;
            #endif 
