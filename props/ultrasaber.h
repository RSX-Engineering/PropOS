...
public:
  ...
  bool Event2(enum BUTTON button, EVENT event, uint32_t modifiers) override //__attribute__((optimize("Og"))) 
  {
    ...
    //   case EVENTID(BUTTON_POWER,  EVENT_HELD_LONG, MODE_ON):
    //   {
    //     STDOUT.println("EVENT_HELD_LONG , starting BLE");
    //     #ifdef WIFI_ENABLED_ESP32
    //     if(wifiSerial.getWifiStatus())
    //       wifiSerial.disconnect();
    //     #endif
    //     BLSERIAL.StartAdvBLE();   // Start advertising on BLE 
    //     return true;
    //   }
    //   #endif
      ...
      case EVENTID(BUTTON_NONE, EVENT_TAP, MODE_ON): // toggle STEALTH while on
  ...
  void Loop() override {
    ...
    #ifdef OSX_ENABLE_MTP
      if(MTPS_status::GetSession()) return;    // if media transfer file is active , no prop is running  
    #endif
    
        PropBase::Loop();   // run the prop loop     
        #if defined(SABERPROP) // STM Saberprop and UltraProffies
          if (menu) RequestPower();
        #endif

          SaberBase::RequestMotion(); // request motion 
		      DetectTaps(2);               // run double-tap detection 
  ...
  */
  ...
  void Help() override {
   ...
   void On() override {
    ...
    // #ifdef WIFI_ENABLED_ESP32
    // if(!BLSERIAL.hasClient()) {
    //   BLSERIAL.StopAdvBLE();
    //   wifiSerial.triggerConnToLastCFG();   // Start advertising on BLE 
    // }
    // #else 
