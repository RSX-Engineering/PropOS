        ...
        case 'p':  {  // -- Pixel blade --
            ...
            if (!foundDriver) return false;
                ...
                switch (pixelDriver.driver_type) {
                    #ifdef ARDUINO_ARCH_STM32L4   // STM supports only timer-based driver
                        case 't':
                            STDOUT.print("WS281x timer-based driver on pin DAT");
                        break;
                    #else // ESP supports SPI and RTM drivers
                        case 's':
                        case 't': // TODO: remove this and next line, after updating the install file (that's only to allow PL/PZ install files to work on UP)
                            pixelDriver.driver_type = 's';
                            STDOUT.print("WS281x SPI driver on pin DAT");
                        break;
                        case 'r':
                            STDOUT.print("WS281x RMT driver on pin DAT");
                        break;
                    #endif // architecture
                    default:
                        STDOUT.print("unknown pixel driver ('");
                        char drivertype[2] = {0, 0}; drivertype[0] = pixelDriver.driver_type; 
                        STDOUT.print(drivertype);
                        STDOUT.print("')' on pin DAT");
                        pixelDriver.driver_type = 0;        // mark unknown driver
                }
                STDOUT.print(bladeData.pixelBlade.pin);
                STDOUT.print(" with "); STDOUT.print(pixelDriver.nPixels); 
                STDOUT.print(" pixels, ID = "); STDOUT.print(bladeData.pixelBlade.driverID);  STDOUT.print(" ..."); 
            #endif // DIAGNOSE_BOOT     

            // Check and adjust pixel driver
            switch (pixelDriver.driver_type) {
                case 's': break;        // SPI: do nothing 
                case 'r': break;        // RMT: do nothing
                case 't':               // timer-based 
                    #ifdef ARDUINO_ARCH_ESP32
                         pixelDriver.driver_type = 's'; // convert to SPI for ESP (temporary, until install files are updated)
                    #endif
                    break;
                default: pixelDriver.driver_type = 0;        // mark unknown driver
            }

            if (!pixelDriver.driver_type) {
                #ifdef DIAGNOSE_BOOT
            ...
            WS2811PIN* tmpPinPtr = 0;            
            switch (pixelDriver.driver_type) {
                case 't':   // Initialize timer-based driver (STM only)                    
                    ...
                    #endif // STM Architecture
                break;

                case 's':   // Initialize SPI driver (ESP only)
                    #ifdef ARDUINO_ARCH_ESP32  
                        // STDOUT.print("Creating SPI Driver on hardware pin "); STDOUT.println(pin);
                        tmpPinPtr = new ESPILedPinBase<Color8::Byteorder::GRB>(pixelDriver.nPixels, pin, 1000*pixelDriver.frequency, pixelDriver.reset, pixelDriver.t0h, pixelDriver.t1h);                    
                    #endif 
                break;

                case 'r':   // Initialize RTM driver (ESP only)
                    #ifdef ARDUINO_ARCH_ESP32  
...
bool Install(const char* filename)  {
   ...
   #if defined(SABERPROP)  && defined(SABERPROP_CHARGER)   // && defined(ARDUINO_ARCH_STM32L4)
